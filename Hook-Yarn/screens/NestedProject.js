  ...
  if (!data) {
    return (
      ...
      <View contentContainerStyle={styles.projectsContainer} alignItems={'center'}>  
          <TouchableOpacity
            key={'NewProject'}
            onPress={() => navigation.navigate('NewProject')} // navigation.navigate('ViewNoteScreen', { id: newId })
      ...
      <Button alignItems='bottom' onPress={this.clearAsyncStorage}  title="Delete all uploads">
    ...
    <View style={styles.container}>
      ...
      {filteredProjects.map((item) => (
        <Button
          key={item.id}
          title={item.name}
          onPress={() => navigation.navigate('ViewNoteScreen', {id: item.id})}
        />
      ))}
